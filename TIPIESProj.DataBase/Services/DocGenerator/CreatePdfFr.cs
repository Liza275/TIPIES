...
{
    public class CreatePdfFr
    {
        public static void SaveTable(string nameOfFile, string nameOfDocument, List<TableColumn> columns,
              TableRow[] rows, List<FactRasprReportProductModel> workers, FactRasprFullReportModel model)
        {
        ...
        private static PdfPTable CreateTable(FactRasprFullReportModel model)
        {
            BaseFont baseFont = BaseFont.CreateFont("C:\\Windows\\Fonts\\arial.ttf", BaseFont.IDENTITY_H, BaseFont.NOT_EMBEDDED);
            iTextSharp.text.Font font = new iTextSharp.text.Font(baseFont, 16, iTextSharp.text.Font.BOLD);

            PdfPTable table = new PdfPTable(4);
            table.AddCell(new PdfPCell(new Phrase(TranslitService.Tr2("Всего кол-во"), font)));
            table.AddCell(new PdfPCell(new Phrase(TranslitService.Tr2("Всего продано по фактической цене"), font)));
            table.AddCell(new PdfPCell(new Phrase(TranslitService.Tr2("Всего продано по плановой цене"), font)));
            table.AddCell(new PdfPCell(new Phrase(TranslitService.Tr2("Всего отклонение от плановой стоимости"), font)));
            table.AddCell(new PdfPCell(new Phrase(model.TotalCount.ToString(), font)));
            table.AddCell(new PdfPCell(new Phrase(model.TotalFactPrice.ToString(), font)));
            table.AddCell(new PdfPCell(new Phrase(model.TotalPlannedPrice.ToString(), font)));
            table.AddCell(new PdfPCell(new Phrase(model.TotalOtkloneniePlannedCostPrice.ToString(), font)));

            return table;
        }

        private static PdfPTable CreateTable(List<TableColumn> columns,
            TableRow[] rows, List<FactRasprReportProductModel> workers)
        {
