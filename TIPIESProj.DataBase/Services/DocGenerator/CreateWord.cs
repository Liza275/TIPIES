using Microsoft.Office.Interop.Word;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Reflection;
using System.Text;
using TIPIESProj.DataBase.ViewModels.Report;

namespace TIPIESProj.DataBase.Services.DocGenerator
{
    public class CreateWord
    {
        public static bool CreateWordTable<T>(string path, string documentHeader, List<HelperWordTable> headers, List<T> dataList)
            where T : class
        {
            if (path != null && documentHeader != null && headers != null && dataList != null)
            {
                PropertyInfo[] properties = dataList[0].GetType().GetProperties();

                Application application = new Application();
                application.Visible = false;
                Document document = application.Documents.Add(Visible: false);
                Paragraph paragraphHeader = document.Content.Paragraphs.Add();
                paragraphHeader.Range.Text = documentHeader;
                paragraphHeader.Range.Font.Size = 24;
                paragraphHeader.Range.Font.Name = "Century Gothic";
                paragraphHeader.Alignment = WdParagraphAlignment.wdAlignParagraphCenter;
                Paragraph paragraph = document.Content.Paragraphs.Add();
                paragraph = document.Content.Paragraphs.Add();
                Table table = document.Tables.Add(paragraph.Range, 1, headers.Count);
                table.Borders.Enable = 1;
                table.Range.Font.Name = "verdana";
                table.Range.Font.Size = 10;
                table.Range.ParagraphFormat.Alignment = WdParagraphAlignment.wdAlignParagraphCenter;

                for (int column = 1; column < headers.Count + 1; column++)
                {
                    table.Cell(1, column).Column.Width = headers[column - 1].Width;
                }

                int shift = 0;

                for (int column = 1; column < headers.Count + 1; column++)
                {
                    if (headers[column - 1].AdvancedHeader?.Count != 0 && headers[column - 1].AdvancedHeader != null)
                    {
                        int dop_column = column + shift;

                        table.Cell(1, column).Split(NumRows: 2);
                        table.Cell(1, column).Range.Text = headers[column - 1].MainHeader;
                        table.Cell(2, dop_column).Split(NumColumns: headers[column - 1].AdvancedHeader.Count);

                        for (int j = 0; j < headers[column - 1].AdvancedHeader.Count; j++)
                        {
                            table.Cell(2, dop_column + j).Range.Text = headers[column - 1].AdvancedHeader[j];
                        }

                        shift += headers[column - 1].AdvancedHeader.Count - 1;
                    }
                    else
                    {
                        table.Cell(1, column).Range.Text = headers[column - 1].MainHeader;
                    }
                }

                List<string> headersPropertiesColumns = new List<string>();
                for (int mainHeader = 0; mainHeader < headers.Count; mainHeader++)
                {
                    for (int property = 0; property < headers[mainHeader].Properties.Count; property++)
                    {
                        headersPropertiesColumns.Add(headers[mainHeader].Properties[property]);
                    }
                }

                int rowIndex = 3;

                for (int dataElement = 0; dataElement < dataList.Count(); dataElement++)
                {
                    table.Rows.Add();

                    for (int column = 1; column < table.Columns.Count + 1; column++)
                    {
                        //var data = dataList[dataElement].GetType().GetProperty(headersPropertiesColumns[column - 1]).GetValue(dataList[dataElement]) ?? null;
                        var data = dataList[dataElement].GetType().GetProperty(headers[0].Properties[column - 1]).GetValue(dataList[dataElement]) ?? null;
                        if (data != null)
                        {
                            table.Cell(rowIndex, column).Range.Text = data.ToString();
                        }
                    }

                    rowIndex++;
                }

                document.SaveAs(path);
                document.Close();
                application.Quit();

                return true;
            }

            return false;
        }

        public static bool CreateWordTable<T>(string path, string documentHeader, List<HelperWordTable> headers, List<T> dataList, FactRasprFullReportModel model)
            where T : class
        {
            if (path != null && documentHeader != null && headers != null && dataList != null)
            {
                PropertyInfo[] properties = dataList[0].GetType().GetProperties();

                Application application = new Application();
                application.Visible = false;
                Document document = application.Documents.Add(Visible: false);
                Paragraph paragraphHeader = document.Content.Paragraphs.Add();
                paragraphHeader.Range.Text = documentHeader;
                paragraphHeader.Range.Font.Size = 24;
                paragraphHeader.Range.Font.Name = "Century Gothic";
                paragraphHeader.Alignment = WdParagraphAlignment.wdAlignParagraphCenter;
                Paragraph paragraph = document.Content.Paragraphs.Add();
                paragraph = document.Content.Paragraphs.Add();
                Table table = document.Tables.Add(paragraph.Range, 1, headers.Count);
                table.Borders.Enable = 1;
                table.Range.Font.Name = "verdana";
                table.Range.Font.Size = 10;
                table.Range.ParagraphFormat.Alignment = WdParagraphAlignment.wdAlignParagraphCenter;

                for (int column = 1; column < headers.Count + 1; column++)
                {
                    table.Cell(1, column).Column.Width = headers[column - 1].Width;
                }

                int shift = 0;

                for (int column = 1; column < headers.Count + 1; column++)
                {
                    if (headers[column - 1].AdvancedHeader?.Count != 0 && headers[column - 1].AdvancedHeader != null)
                    {
                        int dop_column = column + shift;

                        table.Cell(1, column).Split(NumRows: 2);
                        table.Cell(1, column).Range.Text = headers[column - 1].MainHeader;
                        table.Cell(2, dop_column).Split(NumColumns: headers[column - 1].AdvancedHeader.Count);

                        for (int j = 0; j < headers[column - 1].AdvancedHeader.Count; j++)
                        {
                            table.Cell(2, dop_column + j).Range.Text = headers[column - 1].AdvancedHeader[j];
                        }

                        shift += headers[column - 1].AdvancedHeader.Count - 1;
                    }
                    else
                    {
                        table.Cell(1, column).Range.Text = headers[column - 1].MainHeader;
                    }
                }

                List<string> headersPropertiesColumns = new List<string>();
                for (int mainHeader = 0; mainHeader < headers.Count; mainHeader++)
                {
                    for (int property = 0; property < headers[mainHeader].Properties.Count; property++)
                    {
                        headersPropertiesColumns.Add(headers[mainHeader].Properties[property]);
                    }
                }

                int rowIndex = 3;

                for (int dataElement = 0; dataElement < dataList.Count(); dataElement++)
                {
                    table.Rows.Add();

                    for (int column = 1; column < table.Columns.Count + 1; column++)
                    {
                        //var data = dataList[dataElement].GetType().GetProperty(headersPropertiesColumns[column - 1]).GetValue(dataList[dataElement]) ?? null;
                        var data = dataList[dataElement].GetType().GetProperty(headers[0].Properties[column - 1]).GetValue(dataList[dataElement]) ?? null;
                        if (data != null)
                        {
                            table.Cell(rowIndex, column).Range.Text = data.ToString();
                        }
                    }

                    rowIndex++;
                }

                Paragraph paragraph2 = document.Content.Paragraphs.Add();
                paragraph2 = document.Content.Paragraphs.Add();
                Table table2 = document.Tables.Add(paragraph2.Range, 2, 4);
                table2.Borders.Enable = 1;
                table2.Range.Font.Name = "verdana";
                table2.Range.Font.Size = 10;
                table2.Range.ParagraphFormat.Alignment = WdParagraphAlignment.wdAlignParagraphLeft;
                //table.
                table2.Cell(1, 1).Range.Text = "Всего количество";
                table2.Cell(2, 1).Range.Text = model.TotalCount.ToString();
                table2.Cell(1, 2).Range.Text = "Всего факт стоимость";
                table2.Cell(2, 2).Range.Text = model.TotalFactPrice.ToString();
                table2.Cell(1, 3).Range.Text = "Всего факт затраты";
                table2.Cell(2, 3).Range.Text = model.TotalFactZatrat.ToString();
                table2.Cell(1, 4).Range.Text = "Всего откл от факт себестоимости";
                table2.Cell(2, 4).Range.Text = model.TotalOtkloneniePlannedCostPrice.ToString();

                document.SaveAs(path);
                document.Close();
                application.Quit();

                return true;
            }

            return false;
        }

        public static bool CreateWordTable<T>(string path, string documentHeader, List<HelperWordTable> headers, List<T> dataList, SoldFullReportModel model)
            where T : class
        {
            if (path != null && documentHeader != null && headers != null && dataList != null)
            {
                PropertyInfo[] properties = dataList[0].GetType().GetProperties();

                Application application = new Application();
                application.Visible = false;
                Document document = application.Documents.Add(Visible: false);
                Paragraph paragraphHeader = document.Content.Paragraphs.Add();
                paragraphHeader.Range.Text = documentHeader;
                paragraphHeader.Range.Font.Size = 24;
                paragraphHeader.Range.Font.Name = "Century Gothic";
                paragraphHeader.Alignment = WdParagraphAlignment.wdAlignParagraphCenter;
                Paragraph paragraph = document.Content.Paragraphs.Add();
                paragraph = document.Content.Paragraphs.Add();
                Table table = document.Tables.Add(paragraph.Range, 1, headers.Count);
                table.Borders.Enable = 1;
                table.Range.Font.Name = "verdana";
                table.Range.Font.Size = 10;
                table.Range.ParagraphFormat.Alignment = WdParagraphAlignment.wdAlignParagraphCenter;

                for (int column = 1; column < headers.Count + 1; column++)
                {
                    table.Cell(1, column).Column.Width = headers[column - 1].Width;
                }

                int shift = 0;

                for (int column = 1; column < headers.Count + 1; column++)
                {
                    if (headers[column - 1].AdvancedHeader?.Count != 0 && headers[column - 1].AdvancedHeader != null)
                    {
                        int dop_column = column + shift;

                        table.Cell(1, column).Split(NumRows: 2);
                        table.Cell(1, column).Range.Text = headers[column - 1].MainHeader;
                        table.Cell(2, dop_column).Split(NumColumns: headers[column - 1].AdvancedHeader.Count);

                        for (int j = 0; j < headers[column - 1].AdvancedHeader.Count; j++)
                        {
                            table.Cell(2, dop_column + j).Range.Text = headers[column - 1].AdvancedHeader[j];
                        }

                        shift += headers[column - 1].AdvancedHeader.Count - 1;
                    }
                    else
                    {
                        table.Cell(1, column).Range.Text = headers[column - 1].MainHeader;
                    }
                }

                List<string> headersPropertiesColumns = new List<string>();
                for (int mainHeader = 0; mainHeader < headers.Count; mainHeader++)
                {
                    for (int property = 0; property < headers[mainHeader].Properties.Count; property++)
                    {
                        headersPropertiesColumns.Add(headers[mainHeader].Properties[property]);
                    }
                }

                int rowIndex = 3;

                for (int dataElement = 0; dataElement < dataList.Count(); dataElement++)
                {
                    table.Rows.Add();

                    for (int column = 1; column < table.Columns.Count + 1; column++)
                    {
                        //var data = dataList[dataElement].GetType().GetProperty(headersPropertiesColumns[column - 1]).GetValue(dataList[dataElement]) ?? null;
                        var data = dataList[dataElement].GetType().GetProperty(headers[0].Properties[column - 1]).GetValue(dataList[dataElement]) ?? null;
                        if (data != null)
                        {
                            table.Cell(rowIndex, column).Range.Text = data.ToString();
                        }
                    }

                    rowIndex++;
                }

                Paragraph paragraph2 = document.Content.Paragraphs.Add();
                paragraph2 = document.Content.Paragraphs.Add();
                Table table2 = document.Tables.Add(paragraph2.Range, 2, 3);
                table2.Borders.Enable = 1;
                table2.Range.Font.Name = "verdana";
                table2.Range.Font.Size = 10;
                table2.Range.ParagraphFormat.Alignment = WdParagraphAlignment.wdAlignParagraphLeft;
                //table.
                table2.Cell(1, 1).Range.Text = "Всего продано";
                table2.Cell(2, 1).Range.Text = model.TotalSold.ToString();
                table2.Cell(1, 2).Range.Text = "Всего продано в плановой стоисомти";
                table2.Cell(2, 2).Range.Text = model.TotalSoldInPlannedCostPrice.ToString();
                table2.Cell(1, 3).Range.Text = "Всего сумма";
                table2.Cell(2, 3).Range.Text = model.TotalSoldSum.ToString();

                document.SaveAs(path);
                document.Close();
                application.Quit();

                return true;
            }

            return false;
        }
    }

    public class HelperWordTable
    {
        /// <summary>
        /// <param name="MainHeader">Основной заголовок</param>
        /// </summary>
        public string MainHeader { get; set; }

        /// <summary>
        /// <param name="AdvancedHeader">Дополнительные заголовки</param>
        /// </summary>
        public List<string> AdvancedHeader { get; set; } = new List<string>();

        /// <summary>
        /// <param name="Width">Ширина ячеек</param>
        /// </summary>
        public int Width { get; set; }

        /// <summary>
        /// <param name="Properties">Свойства заголовков</param>
        /// </summary>
        public List<string> Properties { get; set; } = new List<string>();
    }
}
